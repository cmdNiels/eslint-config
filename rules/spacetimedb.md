...
conn.subscriptionBuilder().subscribe(["SELECT * FROM message", "SELECT * FROM room WHERE is_public = true"]);
...
export const PrivateData = table(
	{ name: "private_data", indexes: [{ name: "by_owner", algorithm: "btree", columns: ["ownerId"] }] },
	{
		id: t.u64().primaryKey().autoInc(),
		ownerId: t.identity(),
		secret: t.string(),
